...
6. 個人利用で長期運用する場合は、「OAuth 同意画面」の公開ステータスを **「テスト」のままにせず「本番環境」へ変更する** ことを推奨します。「テスト」のままだと、発行された認証情報（refresh token）が **7 日で自動失効** し、再度ブラウザでの同意が必要になります。「本番環境」に変更すれば無期限になります
...
認証情報が失効している場合（上記の 7 日失効など）は、次回の API 呼び出し時に自動的にブラウザでの再同意フローが開きます。画面の指示に従って Google アカウントで同意し直せば、`data/_config/youtube_token.json` が新しい認証情報で上書きされて復旧します。
...
| `invalid_grant: Token has been expired or revoked` | OAuth 認証情報（refresh token）が失効しています。OAuth 同意画面が「テスト」公開ステータスのままだと 7 日で自動失効するため、個人利用なら「本番環境」へ変更してください（上記「概要欄への反映」参照）。次回の操作時に自動でブラウザの再同意フローが開くので、その場で同意し直せば復旧します |
