...
1. 「縦型ショート動画」の中の **「長い候補を刻んでショートにする」** を開く
...
5. 合計が **10〜180 秒** に収まっていれば **「刻んでショートを作成」** が押せます。選んだ区間が入力順で連結され、1080x1920 の縦型動画になります

提案は `data/{video_id}/shorts/cutplan/cut_{候補ID}.json` に保存され、画面を開き直しても再利用されます（AI は押したときだけ動きます）。提案をやり直したいときは「区間を提案し直す」を押してください。テロップ台本やフックタイトルを付けたい場合は、従来どおり「ショート量産」の導線を使います。
