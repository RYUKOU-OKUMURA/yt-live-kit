...
- [ ] 公開ナビゲーションは「ライブラリ」「取り込み」「設定」の 3 画面だけで、動画詳細は非表示ページとしてライブラリから遷移し、旧「処理済み一覧」は表示されない
...
- [ ] ストレージ容量と全動画の内訳を確認でき、10 件を超えても 11 件目以降へ全件表示・検索またはページングで到達できる。各動画に元動画容量と個別削除導線がある
- [ ] 個別削除では動画識別子・対象 1 件・削除対象バイト数・残る成果物、一括削除では対象件数・総容量・残る成果物を確認ダイアログに表示し、確定前は削除されず、確定後だけ表示した正確な対象が削除される。失敗時は日本語エラーが表示される
- [ ] ストレージ削除後も、チャプター・全文・候補・切り出し済み動画が残る
...
- [ ] 更新成功後だけ概要欄ステップが完了として記録され、更新失敗時は記録されない
- [ ] 旧「処理済み一覧」を含め、差分確認を迂回して概要欄を更新できる経路が残っていない
