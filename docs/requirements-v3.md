...
| **スコープ** | 同時実行ジョブは v2 から変更せず 1 件のまま（キューイングは対象外という NFR は維持）。v3 では、選択した全生成対象の台本を人が確認・確定してから、単一ジョブが順次エンコードする。エンコード中に未確定の次台本を追加する producer-consumer 型の最適化は、ジョブ状態設計を別途要するため次イテレーション候補とする |
