...
## 3. 着手前 Findings（U9 で解消済み）

U9 着手前に発見した構造課題を R2 の P1 / P2 / P3 分類に倣って記録する。P1 / P2 は U9-3 / U9-10 / U9-11 / U9-12 で解消済みである。R2 監査文書 §3「Findings」の P3 リストに記載済みの項目はここに再掲しない。

### P1 — 視覚刷新（第 2 弾）着手前に解消済みの項目

- AppTest 0 件だった状態に対し、U9-3 で主要ページの視覚回帰スモークを追加した
- 候補選択チェックボックスのループ index 依存 key は、U9-11 で `source + candidate.id` へ変更した。重複 ID は既存 service gate でフォーム描画前に日本語 fail closed とする

### P2 — 視覚刷新と同時に解消済みの軽微な項目

- 呼び出し箇所ゼロの `_render_clips` は U9-10 で削除した
- `src/yt_live_kit/ui/pages/` に残っていた git 管理外の `__pycache__` は U9-12 でローカル整理した。コミット差分は発生しない
