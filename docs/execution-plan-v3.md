...
- [ ] U6-12. ショート生産ラインの実機フィードバックを反映する:
  - [ ] U6-12a. `short_cut` の実行中表示・正常終了時の cutplan 再利用・失敗時の元エラー表示を既知の非 pipeline ジョブ契約として回帰確認する
  - [ ] U6-12b. 生成開始前の FFmpeg `subtitles` capability 検査、job / queue snapshot 非作成、工程 4 と台本確認の維持、全件 / 一部失敗表示、mp4 欠損時の工程 5 禁止を回帰確認する。アプリは FFmpeg の導入や `.env` 変更を行わない
  - [ ] U6-12c. 実行中ジョブ名・進捗・処理件数・対象ショート・現在工程・次の確認・経過時間・本日の完了数・要対応件数を左サイドバーへ集約し、メインは操作関連エラー・完成動画・折り畳み時の縮約状態だけにする。重複案内を削除する
  - [ ] U6-12d. 「公開・投稿で予約する」は callback または描画前の遷移要求適用で workspace を切り替え、描画後の widget key 直接代入を行わない。例外時は operation を作らず、動画と確認済みライン状態を維持する
  - [ ] U6-12e. 公開・投稿でタイトル候補選択・タイトル自由編集・説明文編集・タグ追加削除・予約日時確認を可能にする。編集値から新しい preview を構築し、編集後は以前の確認を失効させ、実送信 snapshot を確認ダイアログへ固定表示する。metadata 検証前および最終確定前は operation / 投稿 job を作らない
  - [ ] U6-12f. 対象回帰テストと `uv run pytest` 全件を通し、欠陥優先レビューを完了して大タスクコミットする
