...
| U6 | 動画詳細ページの作業選択型 IA への再構成（v3.1 改訂） | [ ] 未着手 |
| U7 | 概要欄反映の最新性判定 + 素材候補の選択引き継ぎ | [ ] 未着手 |
...
| M15 | 動画詳細が「作業を選んで進めるページ」になる（U6 完了。U7 / U8 で仕上げ） | [ ] |
...
| **U6** | 動画詳細ページの作業選択型 IA への再構成 | 状態サマリー、3 ワークスペース、詳細・再生成、回復用空状態 | FR-17 v3.1, FR-21, AC-31 |
| **U7** | 概要欄反映の最新性判定 + 素材候補の選択引き継ぎ | 反映記録の fingerprint 化、チャプター 6 状態表示、候補の事前選択引き継ぎ | FR-21 v3.1, FR-31, AC-32 |
...
### U6: 動画詳細ページの作業選択型 IA への再構成（v3.1 改訂）

**目的:** 動画詳細ページを「処理パイプラインを上から全部見せるページ」から「実際に行う仕事を選んで進めるページ」へ再構成する（FR-17 v3.1 / AC-31）。
**フェーズ状態:** [ ] 未着手
**前提:** S6 / S7 の完了（同じ `ui/views/shorts.py` / `ui/components/short_cut.py` / `ui/views/settings.py` を触るため、先に S6-8 / S6-9 / S7 を閉じてから着手する）。要件改訂（FR-17 v3.1 / FR-21 v3.1 / AC-31）は 2026-08-01 に完了済み。
...
- **U6 の時点では**概要欄反映状態は既存 `description_applied_videos.json`（ID 配列）による「反映済み / 未反映」の 2 状態でよい。6 状態表示（要再反映・最新性不明）は U7 で追加する
- 既存の確認境界（再生成 / 削除 / 概要欄 before・after / ショート上書き / 予約投稿の各 `st.dialog`）と `render_shorts_queue` の状態契約（snapshot / fingerprint / 確定 / job ID 分離）は**変更しない**。移動と外装の除去だけを行う

**変更ファイル範囲:**
- `src/yt_live_kit/ui/views/video_detail.py`（中核。ステッパー廃止、動画ヘッダー、状態サマリー、作業切り替え、3 ワークスペース条件描画、詳細・再生成、回復用空状態）
- `src/yt_live_kit/ui/views/shorts.py`（外側「縦型ショート動画」expander の除去。量産（`render_shorts_queue`）を主導線に、単体作成・サブ区間提案（`render_short_cut_section`）を補助領域に再配置）
...
- [ ] U6-5. ショート作成ワークスペース: `render_shorts_section` の外側 expander を除去し、量産（`render_shorts_queue`）を先頭の主導線にする。単体作成（区間指定 + 単発生成）とサブ区間提案（`render_short_cut_section`）は「補助」領域として量産の後に配置する（補助領域内での expander 使用は可）
...
**フェーズ状態:** [ ] 未着手
**前提:** U6 完了。2 つの独立した改善だが、どちらも U6 の新 IA を前提とするため同一フェーズにまとめる。着手順は (1) → (2)。
...
P3 完了（0.3.0）
 ...
 ├─ S6 サブ区間提案 ─┐
 └─ S7 FFmpeg 環境検査 ─┴─ U6 動画詳細の作業選択型 IA（v3.1）★S6/S7 と同じ UI ファイルを触るため両者の完了後
                             ├─ U7 反映記録 fingerprint 化 + 候補引き継ぎ
                             └─ U8 構造化エラー通知（U7 と独立・並行可）
```
...
| U6 を S6 / S7 完了後にする | U6 は `ui/views/shorts.py` 等 S6 / S7 と同じファイルを再構成する。進行中タスクと同一ファイルの大規模変更を重ねると、diff の衝突と受け入れ確認の混線を招く |
| U6 → U7 → U8 の 3 分割 | U6（見た目の再構成）と U7（データ形式の移行）を同じ diff に混ぜるとレビュー困難になる。U8 は表示先（詳細・再生成）が U6 で作られるため後続にする |
...
| U6 動画詳細の作業選択型 IA | 2 日 |
| U7 反映記録 fingerprint 化 + 候補引き継ぎ | 1.5 日 |
| U8 構造化エラー通知 | 1 日 |
...
| **0.3.1** | + P4・S6・S7 | — | ショート導線の追加要件とホットフィックス |
| **0.4.0** | + U6 → U7 / U8 | — | 動画詳細が作業選択型 IA（v3.1）になる |
...
| **M15: 動画詳細が「作業を選んで進めるページ」になる（v3.1）** | U6 完了で骨格成立（AC-31 充足）。U7 / U8 完了で AC-32 / AC-33 まで充足し確定 |
...
2. **S6 を閉じる。** 残りは S6-8 のコミットと S6-9 の実機確認（AC-30 の残り 4 項目）
3. **S7 を閉じる。** FFmpeg 字幕フィルタの環境検査と復旧
4. **U6 に着手する。** 動画詳細ページの作業選択型 IA への再構成（FR-17 v3.1 / AC-31）。S6 / S7 と同じ UI ファイルを触るため、必ず両者の完了コミット後に始める
5. U6 完了後、**U7**（反映記録 fingerprint 化 + 候補引き継ぎ、AC-32）と **U8**（構造化エラー通知、AC-33）を進める。U7 / U8 は相互に独立
