...
| U9 | UI 視覚刷新（テーマ適用 + shell 刷新） | [x] 完了 |
| CR-2026-08-06 | 全体コードレビュー修正（F-01〜F-22） | [x] 完了 |
...
**S9-6 実機欠陥 HF5・選択区間音声の開始ずれ修正（2026-08-06）:** 人 preview でユーザーが指摘した「発話とテロップのタイミングが合っていない」の原因を調査し、telop 生成側ではなく `prepare_audio_span` が取得する選択区間音声が要求範囲より 6.06〜9.94 秒早く始まることと特定した。`--download-sections` は keyframe 手前から出力し得るのに、HF3 で入れた `atrim=end_sample` は末尾しか切らないため、長さ検証（frames 数）を通ったまま内容だけがずれていた。調査は [`docs/s9-6-audio-span-offset-2026-08-06.md`](s9-6-audio-span-offset-2026-08-06.md) に分離した。ユーザー承認のうえ**修正方針 1**（ローカル source からの精密切り出し）を採り、fallback にも `--force-keyframes-at-cuts` を足した。`272d5f3` は音声付き `data/{video_id}/clips/source/` があれば `-ss <start> -accurate_seek -i` で切り出す経路を優先し、同じ絶対区間を別 anchor から切り出して PCM を照合する fail closed な開始位置検証、音声 cache schema の `v2` 化、`audio_route` / `alignment` の記録、開始位置を検証できない経路の cache を source 出現時に再生成する契約を追加した。`3455910` は `is_high_precision` に「全音声 span が取得経路を記録していること」を条件として足し、旧 artifact 6 件をファイル移動なしで失効させた。旧 artifact と旧音声 cache は `benchmarks/t1/manifest.json` の不変証跡に含まれるため物理退避はしない（一度退避して T1 テスト 16 件を落とし復元した実績を記録する）。実機の AAC decoder は別 anchor との差が先頭 50 ms 未満に集中するため、検証は先頭 250 ms を warm-up 窓として除外する。開始位置検証の実測 RMS 差は正常な span で 0.060 / 0.207 / 0.090、9 秒ずれた旧 cache との参考比較で 1,010（許容 64.0）だった。`mkw-long-local-asr`（`1b1c8643a89d…`）と `hpe-audio-variation`（`7ef688303557…`）を local_source_accurate_seek 経路で再生成し、前者の cue が宣言区間と一致することを機械照合した。あわせて `hpe-audio-variation` の音声活動記録の誤り（2026-08-06 の「0–28.604 秒が無音」は silencedetect の読み違い。確定値は冒頭 6.19 秒無音で、境界監査の「約 6 秒」と一致する）を訂正し、当方が設定した cut 開始 `02:24:16` が発話の途中で誤りであることを記録した。全体 1839 passed / 2 skipped、`uv lock --check` / `git diff --check` / compile を再確認し、production 15 evidence file は 15/15 不変である。開始ずれは解消したが cue 粒度（16 秒区間が 1 cue）は未解決で T1 の範囲であり、`mkw-long-local-asr` の再確認・`hpe-audio-variation` の opening trim 後 preview・final short の致命的無発話なし確認はいずれもユーザーが行う人確認 gate として未実施のため、S9-6 の全チェック、S9、M16、AC-30 / AC-35 / AC-37 は未完了を維持し、AC-40 も未完了のまま残す。
...
**フェーズ状態:** [x] 完了（U9-1〜U9-12）

**完了順（2026-08-05）:** `U10`（ワークフロー導線の整理）の完了後、U9-1 / U9-4 / U9-5 / U9-7〜U9-9 の視覚仕上げと U9-10〜U9-12 の掃除を再開して完了した。ユーザーの受け入れ基準が「ワークフローとして何をやればいいかが分かりやすくなること」であり、導線構造を先に確定してから視覚仕上げを行った。
...
**リファレンス画像からの意図的な逸脱（2026-08-05 決定）:** リファレンス画像は 6 工程ステッパーをメイン領域の横並びで描いているが、本フェーズでは**サイドバーの縦ステッパーへ移す**。実機確認で、横並びをメイン幅へ収めた結果が同一サイズ・同一色のバッジ 6 個の列になり、現在地・順序・進めない理由のいずれも伝わっていないことを確認したためである。要件書 §FR-33 の「完成時のレイアウトと文言の視覚リファレンスは画像とする。ただし状態遷移・失効・生成条件は本要件書を正本とし、画像と不一致の場合は本文を優先する」に従い、本文の「行き止まりゼロ・毎回選ばせない」を優先する。サイドバーは縦方向に余裕があり、状態アイコン・工程間の接続・次の一手を幅の制約なしに配置できる。

**作業:**

- [x] U9-1. `.streamlit/config.toml` に `[theme]` セクションを追加し、23 オプションのうちリファレンス画像に合わせる配色・文字・形状・チャート項目を設定する。Python コードは変更しない
...
- [x] U9-4. A+ 適用後の実装とリファレンス画像の残差を実測し、差分一覧を更新する。丸数字ステッパー + 接続線 + 鍵アイコン、赤い波下線、KPI カード内の左アイコン配置など「形」の差分を確認する
- [x] U9-5. U9-4 の残差実測を踏まえ、限定 CSS 注入（案 B）は不採用とする。U9-6 の native 縦ステッパーが現在地・一本道・次の一手を満たし、完全な下線タブは見た目だけの差、赤い波下線とテロップ表は `T1-4` の対象であるため、`unsafe_allow_html` 0 件と Streamlit 内部 DOM 非依存を維持する
- [x] U9-6. サイドバーに「作成中のショート」カード・進捗バー・日次カウンタを追加する。`shorts_line.py:947-966` の `st.write` / `st.caption` 実装を置き換える
  - **実装時の変更（2026-08-05）:** 「進捗バー」の代わりに 6 工程の縦ステッパー（完了 ✅ / 進行中 ◉ + 「← 今」/ 未到達 🔒、工程間を `│` で連結）と「── 次にやること ──」の 1 行を置いた。バーは進捗の割合しか伝えないが、受け入れ条件が要求するのは現在地・残りの工程・次の一手であるため。あわせて `render_stage_bar()`（メインの横並びバッジ 6 個）を削除し、工程要約をサイドバーへ一本化した。狭幅・折り畳み時の代替は既存 `render_main_line_summary()` が担う。U9-9 のステッパー刷新は本項で実質的に完了しており、残るのは記号の視覚的強化（案 B 判断後）だけである
  - **`line_stepper_items(None)` の扱い:** ライン未作成（区間列確定前）でも工程 1 を `current` として返す。工程 1「素材選定」は定義上いま取り組む工程であり、全工程を未着手として描くと「現在地が一目で分かる」を満たさないため。表示の導出のみで、`LineState` の生成条件と永続化の意味は変更していない
  - **サイドバー文言の原則:** サイドバーは全ページで描画されるため、特定の画面にいることを前提とした文言（「〜タブで」「〜ページで」）を書かない。`tests/test_ui_view_models.py` で固定している
- [x] U9-7. ヘッダと KPI カード 3 枚を刷新する。全ページ共通のメイン見出しをサイドバーの compact brand へ移し、動画タイトルを Material icon 付き主見出し、KPI を Material icon 付き `st.metric(border=True)` 3 枚にした
- [x] U9-8. ワークスペース切替を Material icon 付きの tab-like な `st.segmented_control` へ整える。conditional rendering、`required=True`、`width="stretch"` を維持し、`st.tabs` と限定 CSS は用いない
- [x] U9-9. 6 工程ステッパーを刷新する。U9-6 で旧 `st.badge` 横並びを削除し、サイドバーの native 縦ステッパーへ一本化した。U9-5 で案 B を不採用としたため追加 CSS は用いない
- [x] U9-10. `video_detail.py:653` の呼び出し箇所ゼロの `_render_clips` を削除する
- [x] U9-11. `shorts_queue.py:502` の候補選択チェックボックス key を配列 index 依存から `source + .id` へ改め、R2-4 の契約に合わせる
- [x] U9-12. `src/yt_live_kit/ui/pages/` に残存する `__pycache__` を削除する。`.gitignore` に `__pycache__/` があり `git ls-files src/yt_live_kit/ui/pages/` は空、すなわち git 管理外であるため、この削除はコミット差分の発生しないローカル整理として実施し、コミット境界には含めない

**Done 条件:**

- [x] `.streamlit/config.toml` の `[theme]` 系設定のみで A+ が適用され、Python コード差分がゼロである
- [x] 新規 `tests/test_ui_visual_smoke.py` が AppTest で主要ページの無例外描画を検証し、全件テストに組み込まれている
- [x] A+ 適用後の残差実測が記録され、案 B の不採用判断が記録されている
- [x] サイドバーの「作成中のショート」縦ステッパー・次の一手・日次カウンタ、ヘッダ、KPI カード、ワークスペース切替、6 工程ステッパーが刷新され、`st.segmented_control` の conditional rendering が維持されている
- [x] **ワークフローの可読性（FR-33 の「行き止まりゼロ・毎回選ばせない」/ AC-35 の一本道に対応する受け入れ条件）**
  - [x] 6 工程のうち現在地が一目で特定でき、完了・進行中・未到達が視覚的に区別される
  - [x] 次にやることが常に 1 つだけ提示される。ライン未開始の空状態でも最初の一歩が示され、行き止まりにならない
  - [x] 進めない工程について、飛べないこと（一本道であること）が表示から伝わる
  - [x] サイドバーとメインが同じ `LineState` から導出され、「作成中のラインはありません」と「進行中」が同時に表示されない
  - [x] 6 工程の完全な要約はサイドバーだけに表示する。メイン側はサイドバー折り畳み時・狭幅時の fallback として 1 行の `工程 N／6` だけを表示する。Streamlit は sidebar の展開状態を Python へ公開しないため、この 1 行は通常幅でも表示されるが、工程名・状態・次の一手は二重表示しない
- [x] `_render_clips` デッドコードと `shorts_queue.py` の index 依存 key が解消されている。`ui/pages/` の `__pycache__` は git 管理外のローカル整理であり、コミット対象の Done 条件には含めない
- [x] R2 §5 の安全境界（transaction 順序、確認 dialog、worker thread からの `st.*` 禁止、`st.tabs` への単純置換禁止）に変更が無い
- [x] 全件 `uv run pytest` が変更前の基準から回帰せずに通る（2026-08-05: 1743 passed / 2 skipped）
...
- [x] U10-1. ショート作成ワークスペースの 3 重選択を解消する。同一クリップが `shorts_line.py:1263` の `st.multiselect`「ショート作成対象（選択順）」→ `shorts_line.py:1286` の `st.radio`「今回作る候補（1 本ずつ進めます）」→ `short_cut.py:1235` の `st.radio`「刻む候補」と、名前の違う 3 つの UI で 3 回選ばされている。とくに `shorts_line.py:1297` の `render_short_cut_section(embedded=True)` は選択済みの 1 件だけを渡しているため、「刻む候補」は**選択肢 1 個のラジオ**になっている。embedded 経路では親候補の再選択 UI を描かず、選択済み候補を確定表示に変える。素材候補ワークスペースからの引き継ぎ（`video_detail.py:998-1005`）で既に対象が決まっている場合に、ショート作成ワークスペースで選び直しを強制しない
  - **実装（2026-08-05）:** 主導線を「今回作る候補: ○○」の確定表示 + 2 件以上のときだけ「この後: 2. ○○」の 1 行にし、`st.multiselect` と `st.radio` を `st.expander("対象を選び直す（全 N 件）")` の中へ移した。実効選択は widget 描画前に session_state から導出し、`line_material_selection_key` / `line_material_target_key` / `line_material_transfer_marker_key` の意味と引き継ぎ失効判定は変更していない。折り畳みは空選択ガードより前に描画する。全解除しても選び直し導線が消えず行き止まりにならないためである。`short_cut.py` は `embedded=True` のとき「刻む候補」の radio を描かず、standalone 経路の `short_cut_parent_{video_id}` にも書き込まない。standalone（`views/shorts.py:492`）は複数の親候補から選ぶ意味があるため現行挙動を維持した
  - **全解除時の扱い:** 折り畳み内で選択が空のとき radio と「選択順:」caption を描かない。空の選択肢で `st.radio` を描くと Streamlit 既定の英語文言「No options to select.」と、対象の無い「選択順:」だけの行が残るためである（実機確認で発見）
- [x] U10-2. 公開・投稿ワークスペースの時間軸を分離する。`video_detail.py:1012-1063` は「元動画の概要欄」（ラインと無関係）・「ショートの予約投稿」（工程 6）・関連動画の Studio 確認（投稿後フォローアップ）を 1 画面に同居させている。加えて `upload.py:908` の `_render_related_video_summary_panel(settings)` は `video_id` を受け取らない全投稿横断パネルであるため、別動画（実機確認では `gZAoIbp4lBc`）の確認待ちが現在の動画の画面に混入する。さらに `upload.py:215-253` の `_render_related_video_status` が同じ確認タスクを重複表示し、片方が「確認待ち一覧は上部パネルをご覧ください」と自己言及している。工程順に沿った区分と、どの動画の作業かの明示、重複パネルの一本化を行う
  - **実装（2026-08-05）:** `_render_publish_workspace` を「ショートの予約投稿」（工程 6）→「投稿後のフォローアップ」→「元動画の概要欄」の 3 見出しへ分け、ラインの時間軸を先に連続させて別トラックの概要欄を最後へ移した。見出しに「いま」「次」の現在地は書かない。現在地の提示はサイドバーの縦ステッパー（U9-6）の役割であり、二重表示しないためである
  - **重複の一本化:** 現在の動画の確認タスクは `_render_related_video_status` を唯一の入口とし、全体パネルにあった確定ボタンをここへ移して自己言及 caption を削除した。`ui/view_models/upload.py:26` の `_is_actionable` が `related_video_status == "pending"` を無条件に actionable と判定するため、現在の動画の pending は必ず `project_source_operations` に載る。よって全体パネルから現在の動画を除外しても追跡は失われない
...
## CR-2026-08-06: 全体コードレビュー修正

**調査記録:** [`docs/code-review-2026-08-06.md`](./code-review-2026-08-06.md)

**確定方針（ユーザー承認）:**
- F-01: UI 単発 mp4 切り出しは正式廃止。CLI `clips cut` は維持
- F-03: OAuth 未設定運用はサポート終了。長尺 `build_description` 系は削除
...
| 日付 | 内容 |
|------|------|
| 2026-08-08 | **macOS のキャッシュ掃除で消滅した Whisper モデル実体を復旧した。** `~/Library/Caches/whisper.cpp/` と `~/Library/Caches/yt-live-kit/` がディレクトリごと消滅し、`ui/runtime_checks.check_whisper_model_warning` が起動時警告を出していた（コード不具合ではない）。モデル実体は VoiceInk（`com.prakashjoshipax.VoiceInk/WhisperModels/`）に残存しており、SHA-256 が `config.WHISPER_MODEL_SHA256`（`394221709c...`）と完全一致することを確認した上でコピー（VoiceInk 側は不変）した。`.env` の `YTLK_WHISPER_MODEL_PATH` を再消滅しない `~/Library/Application Support/yt-live-kit/whisper/models/` へ向け直し、加えて `benchmarks/t1/manifest.json:308` が不変証跡として記録している旧 Caches パス（`~/Library/Caches/whisper.cpp/models/...`）にも実体を復元し、T1 の `_check_source_file` 検証が失敗していた 9 件を解消した。manifest は改竄せず、記録された path への実体復元だけを行う方針を採った（記録された bytes / SHA-256 は実体と完全一致しており、証跡自体は正しかった）。回帰 **1909 passed / 2 skipped**（修正前は T1 起因で 1900 passed / 9 failed / 2 skipped）、ruff pass、`uv run mypy src` 0 errors。`config.py` の contract 定数・既定値、`data/`、`benchmarks/`（manifest.json 含む）は不変。**教訓:** OS のキャッシュ掃除は `~/Library/Caches/` 配下のアプリ専用ディレクトリを丸ごと消しうるため、外部実体（モデル・証跡参照パス）をキャッシュ配下だけに置くと再発する。再発時は同じ手順（実体の別所在確認 → SHA-256 照合 → コピーで復元）を踏む |
| 2026-08-07 | **実運用で高精度字幕が失敗する CR-2026-08-06 起因の regression を修正した。** ユーザーが実 UI で `WVaA_i7uH9Q` の選択区間を高精度化しようとして「選択区間の高精度字幕に失敗しました」（code `model_mismatch`）が発生。原因は `28d2491` が `WHISPER_MODEL_PATH` の既定をマシン固有の絶対パス（`~/Library/Caches/whisper.cpp/models/...`）から移植可能な `./data/_config/whisper/models/...` へ変更した際、**モデル実体（547 MB）の配置手順も `.env` 記載も伴わなかった**こと。既定パスの変更自体はコードレビューの正当な指摘への対応なので値は戻さず、実体の場所を `.env` の `YTLK_WHISPER_MODEL_PATH` で指定して即時復旧した（実体の SHA-256 が `config.WHISPER_MODEL_SHA256` と一致することを照合済み）。恒久対策として、(1) `whisper_runtime._file_fingerprint` / `_resolved_executable` の診断メッセージへ設定パスと設定すべき環境変数名を含めた（`services/ytdlp.py` の FFmpeg 先例に揃えた。従来は「model が見つかりません」だけで**どのパスを見たか分からず**、これが診断コストの直接原因だった）、(2) `ui/runtime_checks.check_whisper_model_warning` を追加し起動時に警告（SHA-256 は計算せず存在確認のみ。rerun ごとに 547 MB を読まないため）、(3) `.env.example` を新規追加、(4) README に初回セットアップ手順とトラブルシュート行を追加、の 4 点を実施した。回帰テスト 8 件追加で **1909 passed / 2 skipped**、ruff pass、mypy 0。`config.py` の contract 定数・既定値、`data/`、`benchmarks/` は不変。**教訓:** 型検査と単体テストが全緑でも、外部実体のパス設定は実運用でしか露見しない |
| 2026-08-07 | **main を origin へ push し、CI を初めて実行して緑にした（Ruff / Mypy / Pytest すべて ✓）。** 初回実行で `17 failed` が露見したが、原因は CR-F18 の変更ではなく既存の問題だった（当該 commit は `benchmarks/` を一切触っていない）。`benchmarks/t1/annotation_packet.py` の `PRODUCTION_DATA_ROOT` が開発者ローカルの絶対パス（`/Users/.../yt-live-kit/data`）を直指ししており、`data/` は OAuth トークンを含むため gitignore 対象で、CI にも他マシンにも存在し得ないことによる。該当 17 件へ production データ実在を条件とする `skipif` を付けた（`tests/test_shorts.py` の実 ffmpeg 統合テスト skip と同じ前例に倣う）。skip 集合が CI の失敗集合と完全一致することを差分照合で確認しており、CI で走り得るテストは 1 件も skip していない。T1-1 の frozen な証跡（`FORMAL_MANIFEST_FREEZE_COMMIT` / `PRODUCTION_AFTER_ARTIFACT_SHA256` / `MANIFEST_SCHEMA`、`docs/benchmarks/`、`benchmarks/`、`src/`）は不変。件数は **ローカル 1901 passed / 2 skipped**、**CI 1884 passed / 19 skipped** で、1884 + 17 = 1901 と整合する。**残課題:** これらの benchmark 検証は設計上ローカル専用であり、CI で production 整合性を継続検証する仕組みは無い |
...
| 2026-08-06 | **CR-2026-08-06 完了。** [`docs/code-review-2026-08-06.md`](./code-review-2026-08-06.md) の F-01〜F-22 を Composer 2.5 サブエージェントで実装。F-01/F-03 は廃止方針、回帰 1889 passed。CI（ruff+pytest）導入 |
