...
| S8 | 区間内容の可視化 + プレビュー幅修正（v3.2・最優先） | [ ] 未着手 |
| U6 | ショート生産ライン UI（v3.2 改訂: 作業選択型 IA + 工程 UI） | [ ] 未着手 |
...
| **U6** | ショート生産ライン UI | 3 ワークスペース骨格 + ショート 1 本の工程 UI（ゲート・接続・既定値） | FR-17 v3.1, FR-33, AC-31, AC-35 |
...
- [ ] S8-1. 区間テキスト抽出の純粋関数を `ui/components/short_cut.py` に実装する: `(vtt cues, start_ms, end_ms) -> 表示用テキスト`。既存の `parse_vtt_with_end()` + `filter_cues_for_segment()` + progressive 重複除去を再利用し、読みやすい行のリストへ整形する。VTT の読み込みは 1 回だけ行い、rerun ではキャッシュ（`st.session_state` または `st.cache_data`）を使う
- [ ] S8-2. `_render_plan()` の各候補カードに、現在の境界入力値（`start_key` / `end_key` の編集後の値）に追従する文字起こしを表示する。境界が不正な間は元の提案区間のテキストを表示し、日本語でその旨を示す
- [ ] S8-3. `st.video` の幅を制限する（4 箇所）。縦 9:16 動画が読みやすいサイズ（目安: 幅 320〜400px 相当。`st.video(width=...)` または列レイアウト）で表示されるようにする
- [ ] S8-4. ユニットテスト: 区間抽出（境界一致 / 部分重なり / 空区間）、重複除去、境界変更への追従、不正境界時のフォールバック
- [ ] S8-5. `uv run pytest` 全件通過。実機確認: 660 秒級候補 → 提案 → **テキストを読んで境界を調整** → 生成 → 話が途中で切れていないことを確認（S6-9 の残項目もここで消化: 通常 rerun で Codex が再実行されないこと、採否チェックと合計尺表示）。進捗チェック更新・コミット

**Done 条件:**

- [ ] AC-34 の全項目が満たされている
- [ ] AC-30 の残項目（採否チェック付き表示・合計尺と作成可否・Codex 非再実行・確定区間からの生成）が実機で確認され、S6 がクローズできる
...
**目的:** 動画詳細ページを「作業を選んで進めるページ」へ再構成し（FR-17 v3.1 / AC-31）、その中核であるショート作成ワークスペースを **FR-33 のショート生産ライン（工程 UI）** として実装する（AC-35）。
**フェーズ状態:** [ ] 未着手
**前提:** S8 完了（ゲート①の確認材料が先に必要）。要件は FR-17 v3.1 + FR-33 / AC-31 + AC-35（2026-08-01 改訂済み）。
...
**背景:** UX 監査（[`.codex/audits/video-detail-ux/audit.md`](../.codex/audits/video-detail-ux/audit.md)）で次の実測が出た。(1) 1280×720 環境でショート作成見出しまでスクロール 2,418px、(2) ステッパー CTA を押しても画面下方の expander が開くだけで結果が見えない、(3) 同じ CTA がステップごとにページ遷移 / ダイアログ / expander 展開 / API プレビュー開始と異なる動作をする、(4) 予約投稿（`ui/components/upload.py`）は量産 manifest しか読まないため単体生成ショートは投稿できないが、その制約が画面から読み取れない。独立に行った Claude 側のコードレビューも同じ結論（縦積み + 入れ子の廃止、チャプターの概要欄への統合、量産主導線化）に到達しており、両者の一致点を本タスクの設計とする。
...
- 選択状態の key は必ず動画 ID を含める（動画 A / B の混在防止。FR-17 v3.1 の初期選択規則を純粋関数として実装しテストする）
- 状態カードはクリック不可・ボタン非配置。移動の入口は作業切り替えのみ（入口の二重化を避ける）
- 概要欄反映状態は既存 `description_applied_videos.json`（ID 配列）による「反映済み / 未反映」の 2 状態でよい。6 状態表示（要再反映・最新性不明）は U7 とともに保留（優先度③: チャプターは保守のみ）
- 既存の確認境界（再生成 / 削除 / 概要欄 before・after / ショート上書き / 予約投稿の各 `st.dialog`）と `render_shorts_queue` の状態契約（snapshot / fingerprint / 確定 / job ID 分離）は**変更しない**。工程のゲートとして再利用する
...
- **既定値（毎回選ばせない）:** レイアウト・通常 / Hook プリセットは `ui/views/_local_settings.py` の既定値読み取り関数（P5 で編集 UI を追加。ファイル未設定時は現行既定 = blur / 既定プリセット）を使い、工程中の選択 UI を出さない
...
- `tests/test_ui_shorts.py` / `tests/test_ui_highlights.py` / `tests/test_ui_upload.py` / `tests/test_ui_shorts_queue.py`（expander / 見出し前提の期待値を新しい表示契約へ更新。確定・上書き・job ID 分離の非回帰確認）
- `README.md`（動画詳細ページの説明を新構成へ更新）
- （**`services/` は一切変更しない。** フェーズ U の原則に戻る）

**作業:**

- [ ] U6-1. 状態計算の作り替え: `calculate_progress_steps()`（5 段ステッパー用）を廃止し、状態サマリー用の純粋関数（素材候補数 = 切り抜き + ハイライト件数、生成ファイル本数 = `count_shorts()`、予約可能本数 = 最新の検証済み量産 manifest の成功 item のうち出力ファイル存在数、概要欄反映状態）と、FR-17 v3.1 の初期選択規則（6 規則）を返す純粋関数を実装しテストする
- [ ] U6-2. 動画ヘッダー + 状態サマリー + 作業切り替えを実装する。`st.segmented_control` の key は `f"detail_workspace_{video_id}"` 形式とし、初期値は U6-1 の規則で決める。切り替えは描画の切り替えのみで副作用を持たない
- [ ] U6-3. 回復用空状態: `load_result_from_disk()` が `None` の場合、通常ワークスペースを描画せず「保存済みの字幕成果物を読み込めませんでした」+ 原因説明 + 「取り込みで再処理」CTA（`st.switch_page` で取り込みへ。ジョブは開始しない）+ 詳細（動画 ID・存在する成果物）を表示する
- [ ] U6-4. 素材候補ワークスペース: 切り抜き候補とハイライト候補をカード表示し、両方ある場合のみソース切り替えを出す。候補の生成・再生成は空状態または末尾の補助操作に置く。ハイライトまとめ動画の生成導線（`render_highlights_section` の生成部分）は本ワークスペース内の補助として維持する
- [ ] U6-5. 工程 UI の骨格: 6 段階の工程表示と、ゲート通過判定（区間確定済みか / テロップ確定済みか / 生成物存在 + プレビュー確認済みか）の純粋関数を実装しテストする。工程状態は動画 ID + 作成中アイテム単位の session state で持ち、成果物ファイル（cutplan / telop / mp4 / queue record）を正本として再起動後も工程位置を復元できるようにする
- [ ] U6-5b. 接続の実装: カットプラン確定区間列 → テロップ生成ジョブ → 人の確認・修正（S4 の確定機構を流用）→ `build_short_from_segments()` 生成 → プレビュー → 予約導線（公開・投稿ワークスペースの予約フローへ）。レイアウト・プリセットは既定値を使う。量産・単体作成は補助領域へ移す（補助領域内での expander 使用は可）
- [ ] U6-6. 公開・投稿ワークスペース: (a) 元動画の概要欄カード — チャプター状態（U6 では生成済み・件数・形式 OK / 未生成 / 形式エラーと反映済み / 未反映）+ 「概要欄に反映」ボタン（既存 `_start_description_preview` → before / after ダイアログのフローを維持）。チャプター本文は表示しない。(b) ショートの予約投稿カード — `render_upload_section` を統合し、予約可能 0 本かつ生成ファイルありの場合は「生成済みですが予約対象に追加されていません」+ 量産フローへの導線、両方 0 本の場合は「先にショートを作成してください」+ ショート作成への導線を表示する
- [ ] U6-7. 詳細・再生成: 最下部の `st.expander("詳細・再生成", expanded=False)` 1 つに、字幕全文 + 全文コピー、チャプター本文 + タイムラインコピー（OAuth 未設定時の手貼りフォールバックとして維持）、チャプター / 切り抜き候補の再生成、元動画と中間ファイルの管理を集約する。各確認ダイアログは既存のまま維持する
- [ ] U6-8. テスト更新: U6-1 の純粋関数（状態サマリー・初期選択の各分岐）、ワークスペース条件描画、回復用空状態、予約可能 / 生成ファイルの区別、既存確認ダイアログの非回帰
- [ ] U6-9. `uv run pytest` 全件通過、実ブラウザで初期表示・3 ワークスペース・回復用空状態・概要欄プレビュー・予約投稿確認を目視確認し、README・進捗チェックを更新してコミットする

**Done 条件:**

- [ ] AC-31 の全項目（工程 UI 前提で読み替えるもの以外）と AC-35 の全項目が満たされている
- [ ] 初期表示で字幕全文・チャプター本文が描画されず、選択中の 1 ワークスペースだけが描画される
- [ ] 作業切り替えに副作用が無く、実処理はワークスペース内の明示ボタンからのみ開始される
- [ ] ショート 1 本が工程の一本道（刻む → テロップ → 生成 → 確認 → 予約)で完成し、YouTube 自動字幕の生焼き込みが工程上発生しない
- [ ] 既存の確認ダイアログ・S4 の状態契約・FR-21 の安全契約に回帰が無い
- [ ] `services/` に変更が無い（`git diff --stat` で確認。不足が判明した場合は実装前に計画改訂へ戻した記録がある）
- [ ] `uv run pytest` が全件通る
...
> **v3.2 注記（2026-08-01）:** 運用目標の優先順位（§1.3.1: チャプター反映は優先度③・保守のみ）により、fingerprint 化（FR-21 v3.1 / AC-32 前半）は **v4 候補へ保留**する。素材候補の選択引き継ぎ（FR-31 / AC-32 後半）は **U6 の工程接続に統合済み**で、独立タスクとしては消滅した。以下の定義は保留時点の記録として残す。着手する場合は U6 完了後。

**目的:** (1) チャプター再生成後も「反映済み」に見える実データ上の不整合を、反映記録の fingerprint 化で解消する（FR-21 v3.1 / AC-32 前半）。(2) 素材候補ワークスペースで選んだ候補をショート作成の未確定フォームへ事前選択として引き継ぐ（FR-31 / AC-32 後半）。
**フェーズ状態:** [保留]
**前提:** U6 完了。
...
- `src/yt_live_kit/ui/components/shorts_queue.py`（引き継ぎ選択（動画 ID 別 session state: 候補ソース / 選択 ID 配列 / 候補ファイル fingerprint / 選択順）を未確定フォームの事前選択として受け取る境界を追加。正式 snapshot は従来どおり「選択内容を確定」時のみ作る）
- `src/yt_live_kit/ui/views/video_detail.py` または素材候補描画部（候補カードに「ショート作成対象へ追加」導線）
- `tests/test_ui_video_detail_page.py`（6 状態の表示分岐、fingerprint 一致 / 不一致 / 旧形式）
- `tests/test_ui_shorts_queue.py`（事前選択の反映、fingerprint 失効時の破棄と日本語案内、確定時のみ snapshot 生成、既存契約の非回帰）
...
- [ ] U7-4. 素材候補カードに「ショート作成対象へ追加」を実装する。引き継ぎ状態は動画 ID 別 session state のみで持ち、ソース別に選択を保持する。ショート作成を開いたとき該当候補を事前選択済みで表示する
- [ ] U7-5. 失効処理: 候補ファイルの fingerprint 変更・候補再生成・選択 ID 不在のいずれかで引き継ぎ選択と未確定 snapshot を破棄し、日本語で再選択を案内する
- [ ] U7-6. テストを追加し、`uv run pytest` 全件通過・実ブラウザ確認（チャプター再生成 → 要再反映表示 → 再反映 → 最新反映済み表示、候補追加 → ショート作成で事前選択）・進捗チェック更新・コミット

**Done 条件:**

- [ ] AC-32 の全項目が満たされている
- [ ] 旧形式の反映記録を持つ既存 47 本が読み込みエラーにならず「最新性不明」と表示される
- [ ] FR-26 の既存契約（fingerprint 失効・確定・上書き確認・job ID 分離）に回帰が無い
- [ ] `services/` に変更が無く、`uv run pytest` が全件通る
- [ ] タスク完了コミット済み
...
| P5 を U6 の後にする | 既定値の読み取りは U6 で先行し（ファイル未設定時は現行既定）、編集 UI と枠拡張を P5 でまとめる。U6 が `services/` 不変更、P5 が `services/schedule.py` 変更ありと、レイヤ制約の異なる作業を別 diff に分ける |
...
| 種類 | 対象 | タイミング |
|------|------|------------|
| ユニット | ステッパー計算（U2。U6 で状態サマリー計算・初期選択規則へ置換）、`_local_settings` の永続化、テロップ台本バリデータ、累積タイムオフセット計算、`assign_next_slot`、metadata / audience / synthetic / consent、resumable retry、operation / attempt / confirm race、kind 別 status bar、反映記録 fingerprint（U7）、構造化エラー通知（U8） | 各タスク内 |
...
2. **S8 に着手する（最優先）。** 区間内容の可視化 + プレビュー幅修正（FR-34 / AC-34）。実機確認で S6-9 を吸収し、S6 をクローズする
3. **U6 に着手する。** ショート生産ライン UI（FR-17 v3.1 + FR-33 / AC-31 + AC-35）
