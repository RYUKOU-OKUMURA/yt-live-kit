...
| U3 | 取り込みページ | [x] 完了 |
| U4 | 設定ページ | [x] 完了 |
| U5 | 正式 4 画面 IA + ストレージ管理移設 + 概要欄差分プレビュー + フェーズ U 受け入れ | [ ] 未着手 |
...
- [ ] 反映前 → 反映後の対比が画面に表示される
- [ ] 公開ナビゲーションが 3 画面 + 非表示詳細の合計 4 画面になり、`history.py` / `test_ui_history_page.py` / `url_path="history"` が残っていない
- [ ] ストレージ管理が設定ページへ移り、v1 / v2 の AC-15 に回帰がない
- [ ] update 成功後だけ mark されること、update 失敗時に mark されないこと、mark 単独失敗時の日本語警告を自動テストで確認済み
- [ ] 実 YouTube 書き込みを受け入れ試験で実行していない
- [ ] `uv run pytest` が全件通る
