...
| **U5** | 概要欄反映の差分プレビュー + フェーズ受け入れ | 差分表示、確認導線統一 | FR-21, AC-22, AC-18〜22 全体受け入れ |
...
**背景:** 現状の [`ui/views/history.py`](../src/yt_live_kit/ui/views/history.py)（U0 移設後）の `_render_description_preview` は「反映後」のマージ済みテキストのみを `st.code` で表示している。反映前の現在値は取得しているが画面に出していない。

**作業:**

- [ ] U5-1. 反映前（`fetch_video_snippet` の `description`）と反映後（`merge_chapters_into_description` の結果）を **並べて、または前後比較の形で** 表示する
- [ ] U5-2. 「概要欄に反映」ボタンを他の操作ボタンと異なる強調色（`type="primary"` に加えて警告色のキャプション）にする
- [ ] U5-3. 確認は `st.dialog` に統一する（U2-4 のパターンを踏襲）
- [ ] U5-4. **フェーズ U 受け入れ:**
...
- [ ] AC-18〜AC-22 が満たされている（未達は明示的に申し送りする）
- [ ] 反映前 → 反映後の対比が画面に表示される
- [ ] `uv run pytest` が全件通る
- [ ] フェーズ完了コミット済み
...
| P3 フェーズ P 受け入れ | 1 日 |
| **合計** | **約 17 日**（1 人、AI 支援あり。P0 の審査待ち日数は含まない） |

**段階リリース案:**

| リリース | 含むタスク | 累計 | 内容 |
|----------|-------------|------|------|
| **0.2.1** | U0〜U5 | 6 日 | 迷わない UI に生まれ変わる |
| **0.2.2** | + S1〜S5 | 13 日 | テロップ付きショートが量産できる |
| **0.3.0** | + P0〜P3 | 17 日 | 予約投稿まで含む v3 完了 |
...
| **M12: 動画軸で迷わず作業できる新 IA が揃う** | U5 完了。4 ページ構成が完成し、ステッパー・確認ダイアログ・差分プレビューが動く |
...
| 実機結合 | 公開アーカイブ 1 本（U5 / S5）、実アップロード 1 本（P0 / P3） | U5 / S5 / P0 / P3 |
