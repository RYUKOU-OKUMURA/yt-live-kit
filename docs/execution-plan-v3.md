...
- [x] H1-2. video ID の path confinement。`services/_paths.py` を新設し、`services/ytdlp.py` / `history.py` / `jobs.py` / `shorts_queue.py` / `shorts_line.py` / `upload_queue.py` と対応テストを段階移行した。通常の 11 文字 ID は互換、`.` / `..` / separator / root 外 symlink は副作用前に日本語で拒否し、解決後 path が `data_dir` 配下であることを確認した。識別子の side-effect-before-rejection、resolved path、manifest / upload snapshot、yt-dlp 出力内 symlink の境界テストを追加し、`uv run pytest -q` は `1162 passed, 2 skipped`、独立レビューは P0/P1 なし。タスク単位コミット済み。見積もり 1 日
