...
- [ ] **ワークフローの可読性（FR-33 の「行き止まりゼロ・毎回選ばせない」/ AC-35 の一本道に対応する受け入れ条件）**
  - [ ] 6 工程のうち現在地が一目で特定でき、完了・進行中・未到達が視覚的に区別される
  - [ ] 次にやることが常に 1 つだけ提示される。ライン未開始の空状態でも最初の一歩が示され、行き止まりにならない
  - [ ] 進めない工程について、飛べないこと（一本道であること）が表示から伝わる
  - [ ] サイドバーとメインが同じ `LineState` から導出され、「作成中のラインはありません」と「進行中」が同時に表示されない
  - [ ] 同じ工程要約をサイドバーとメインへ二重に常時表示しない。メイン側はサイドバー折り畳み時・狭幅時の代替に限る
