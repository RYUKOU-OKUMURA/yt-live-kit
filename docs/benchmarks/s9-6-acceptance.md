...
詳細・再現手順・修正方針の候補は [`docs/s9-6-audio-span-offset-2026-08-06.md`](../s9-6-audio-span-offset-2026-08-06.md) に分離した。修正は production code の変更であり S9-6 の宣言済み変更ファイル範囲外のため、着手していない。

## 既存記録の訂正

上の `hpe-audio-variation` の音声活動実測は、**ずれた cache wav を対象に測っていたため誤り**である。source 8640 起点の精密シークで測り直した正しい値は次のとおりである。

- 誤: 冒頭 1.194 秒無音 → 4.115–16.131 秒に 12 秒の無発話 → 持続発話開始は 16.1 秒
- 正: **0 – 28.604 秒が無音**。以降も 30.431–33.872 秒に 3.4 秒、34.500–35.615 秒に 1.1 秒と断続する

候補冒頭の無発話は約 28.6 秒であり、境界監査の「約 6 秒」も当方の「16.1 秒」も過小評価だった。当方が設定した cut 開始 02:24:16 は無発話区間の内側で誤りである。`mkw-long-local-asr` 側の実測は source 由来の精密シークで行っており、この誤りの影響を受けない。

## 残タスク

1. 選択区間音声の開始ずれを修正する（別承認が必要）
2. 修正後に既存の高精度 artifact を失効させ、`mkw-long-local-asr` を再生成してタイミングを再確認する
3. `hpe-audio-variation` を正しい区間で生成し、opening trim 後 preview を行う
4. final short に致命的な無発話がないことを確認する
